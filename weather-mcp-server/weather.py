...
# ============== SAMPLING DEMO ==============
# Tool that demonstrates sampling - server asking client for LLM assistance

@mcp.tool()
async def analyze_weather_pattern(cities: str, ctx: Context) -> str:
    """Analyze weather patterns across multiple cities using AI assistance.
    
    This tool demonstrates MCP sampling - the server requests the client
    to make an LLM call to analyze complex weather data.
    
    Args:
        cities: Comma-separated city names (e.g., "New York, Los Angeles, Chicago")
    """
    # Parse cities
    city_list = [c.strip() for c in cities.split(",")]
    valid_cities = [c for c in city_list if c in EXAMPLE_CITIES]
    
    if not valid_cities:
        available = ", ".join(EXAMPLE_CITIES.keys())
        return f"No valid cities found. Available: {available}"
    
    # Fetch weather data for all cities
    weather_data = {}
    for city in valid_cities:
        city_info = EXAMPLE_CITIES[city]
    ...
    if not weather_data:
        return "Unable to fetch weather data for the requested cities."
    ...
    for city, data in weather_data.items():
        ...
        for period in data['forecast']:
            weather_summary += f"  - {period['name']}: {period['temp']}, {period['wind']}, {period['description']}\n"
    
    # Use sampling to ask the client's LLM for analysis
    # This is the key part - the server requests AI assistance from the client
    try:
        ...
        logger.warning(f"DEBUG: Client params: {client_params}")
        if client_params:
            caps = client_params.capabilities
            logger.warning(f"DEBUG: Client capabilities: {caps}")
            logger.warning(f"DEBUG: Sampling capability: {caps.sampling if caps else 'None'}")
        else:
            logger.warning("DEBUG: No client params available")
            ...
            messages=[
                SamplingMessage(
                    ...
                    content=MCPTextContent(
                        type="text",
                        text=f"""Please analyze the following weather data and provide insights:

{weather_summary}

Provide:
1. A comparison of conditions across cities
2. The best city for outdoor activities today
3. Any notable weather patterns or concerns
4. Travel recommendations between these cities
...
Keep the analysis concise but informative."""
            ...
            max_tokens=500,
            system_prompt="You are a helpful weather analyst. Provide clear, actionable insights about weather patterns."
        )
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Note: This analysis was generated by requesting the client's LLM
to process the weather data (MCP Sampling feature).
"""
...
- The sampling request was rejected
